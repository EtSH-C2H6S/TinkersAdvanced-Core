...
public class FakeExplosionUtil {
    public static void fakeExplode(float damage, @NotNull LivingEntity causingEntity, @NotNull Level level, @NotNull Vec3 position, @NotNull Predicate<Entity> predicate, boolean setThorn){
        ...
        for (Entity entity:list){
            if (entity!=null&&!entity.isInvulnerableTo(damageSource)&&!entity.isInvulnerable()&& predicate.test(entity)&&!(entity instanceof ItemEntity)&&!(entity instanceof ExperienceOrb)){
    ...
    public static void fakeExplode(float damage,@NotNull LivingEntity causingEntity,@NotNull Level level,@NotNull Vec3 position,@NotNull IntOpenHashSet blackListEntityIds,boolean setThorn){
        fakeExplode(damage,causingEntity,level,position,entity -> !blackListEntityIds.contains(entity.getId()),setThorn);
    }
}
